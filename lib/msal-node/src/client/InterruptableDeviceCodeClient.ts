...
/**
 * OAuth2.0 Device code client with the stop in the middle of process ability
 */
export class InterruptableDeviceCodeClient extends BaseClient {
  ...
  public async acquireToken(
    ...
    noPolling: boolean = true
  ): Promise<AuthenticationResult | null> {
    const reqTimestamp = TimeUtils.nowSeconds();
    const nullableResponse: ServerAuthorizationTokenResponse | null =
      await this.acquireTokenWithDeviceCode(
        request,
        deviceCodeResponse,
        noPolling
      );

    if (nullableResponse === null) return null;

    const response: ServerAuthorizationTokenResponse = nullableResponse;
  ...
  private async acquireTokenWithDeviceCode(
    request: CommonDeviceCodeRequest,
    deviceCodeResponse: DeviceCodeResponse,
    noPolling: boolean = false
  ): Promise<ServerAuthorizationTokenResponse | null> {
    ...
    while (
      ...
      if (response.body && response.body.error) {
        ...
        if (response.body.error === Constants.AUTHORIZATION_PENDING) {
          ...
          if (noPolling) return null;
          else await TimeUtils.delay(pollingIntervalMilli);
        } else {
