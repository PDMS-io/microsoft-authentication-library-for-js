...
import { InterruptableDeviceCodeClient } from './InterruptableDeviceCodeClient.js';
  ...
  implements IPublicClientApplication
{
  ...
  /**
   * Acquires the device code that can be temporarily stored somewhere and then be utlized by another routine
   * to complethe the authentication
   */
  public async acquireDeviceCode(
    request: DeviceCodeRequest
  ): Promise<DeviceCodeResponse | null> {
    this.logger.info('acquireDeviceCode called', request.correlationId);
    const validRequest: CommonDeviceCodeRequest = Object.assign(
      request,
      await this.initializeBaseRequest(request)
    );
    ...
    try {
      ...
      const deviceCodeClient = new InterruptableDeviceCodeClient(
        deviceCodeConfig
      );
      this.logger.verbose(
        'Device code client created',
      ...
      return await deviceCodeClient.getDeviceCode(validRequest);
    } catch (e) {
      if (e instanceof AuthError) {
        e.setCorrelationId(validRequest.correlationId);
      }
   ...
   * Acquires a token from the authority using OAuth2.0 device code flow.
   */
  public async acquireTokenWithDeviceCode(
    request: DeviceCodeRequest,
    ...
    try {
      ...
      return await deviceCodeClient.acquireToken(
        validRequest,
        deviceCodeResponse,
        true
      );
